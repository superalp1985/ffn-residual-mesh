...
The file structure and local SHA256 were checked in place. The digest has NOT
been matched against a trusted upstream hash; download-source integrity is
...
GGUF total blocks: 65 (one MTP block is excluded from the 64 main layers)
```
...
Only main layers `3, 21, 40, 42, 51` currently have both gate and up in
literal `Q4_K` and are eligible for the v1 exact resident compiler. Other
layers remain exact fallback until their actual mixed quantizer has a verified
decoder. This is why the current 8GB claim is a **layer/window prototype**, not
an all-64-layer residency claim.
...
Use `E:\MiniMaxH3\ComfyUI\venv\Scripts\python.exe` for CUDA tests.
`E:\MiniMaxH3\python\python.exe` is the base interpreter without PyTorch.
The actual PyTorch runtime is cu130; the machine's `CUDA_PATH` currently points
to an older toolkit and must not be assumed to identify that runtime.
The prototype uses existing Triton 3.5.1 and added threadpoolctl 3.6.0.
