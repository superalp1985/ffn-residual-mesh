...
2. 冷启动能否把原始 FFN 权重编译成运行时不再完整扫描 `Wg/Wu/Wd` 的展开算子。
3. 紧凑差值包能否触发足够多的 GPU 连续计算并恢复可接受精度。
4. 分层超级块搬运是否能降低 H2D 流量、显存占用和等待时间。
5. CPU 路由成本是否低于节省的 GPU/PCIe 成本。

最高原则：不追求 FLOPs 下降，允许总计算量增加。只以运行时字节下降、GPU 运算/字节上升、GPU 等待占比下降为主要优化方向。
...
- 异常输入能稳定回退，不能让误差跨层失控。
- 正常近似路径对原始 gate/up/down 权重的完整扫描次数为零。
- 报告 `dynamic bytes/token`、`GPU MAC or FLOP/byte` 和 copy/sync 等待时间；不能只报告参数量或 FLOPs。
